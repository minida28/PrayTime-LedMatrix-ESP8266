...
extern uint8_t buffer[HUB08_LCDHEIGHT * HUB08_LCDWIDTH / 8];
//extern uint8_t buffer[BUFFER_HEIGHT * BUFFER_WIDTH / 8];
...
class HUB08 : public Adafruit_GFX {
  public:
    ...
    //uint16_t getPixel(int16_t x, int16_t y, uint16_t buffer);    
    void begin();
    void on();
    void off();
    void scan();
    void scan2();
    void clearDisplay();
    void copyBuffer(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t buf[]);
    //void slideDownDisp(int16_t x, int16_t y, uint16_t w, uint16_t h);
