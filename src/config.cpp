...
#define DEBUGLOG(fmt, ...) \
  { static const char pfmt[] PROGMEM_T = fmt; DBG_OUTPUT_PORT.printf_P(pfmt, ## __VA_ARGS__); }
#else
#define DEBUGLOG(...)
#endif
...
const char hariShortNames_P[] PROGMEM = "ErrMINSENSELRABKAMJUMSAB";
...
time_t local_time() {
  return now() + _config.timezone / 10.0 * 3600;
}

time_t local_time(time_t t) {
  return t + _config.timezone / 10.0 * 3600;
}
...
void process_sholat()
{
  ...
  sholat.get_prayer_times(t_yesterday, _sholatConfig.latitude, _sholatConfig.longitude, _sholatConfig.timezone / 10, sholat.timesYesterday);
  ...
  sholat.get_prayer_times(local_time(), _sholatConfig.latitude, _sholatConfig.longitude, _sholatConfig.timezone / 10, sholat.times);
  ...
  sholat.get_prayer_times(t, _sholatConfig.latitude, _sholatConfig.longitude, _sholatConfig.timezone / 10, sholat.timesTomorrow);
...
char* getLastBootStr() {
  DEBUGLOG("%s\r\n", __PRETTY_FUNCTION__);
  time_t t = _lastBoot + _config.timezone / 10.0 * 3600;
...
void digitalClockDisplay()
{
  ...
  time_t t = now() + _config.timezone / 10.0 * 3600;
